...
Lyrenne is distributed two ways: the portable ZIP from the releases page, and a Scoop
...
No admin rights needed. Scoop puts it under `~\scoop\apps\lyrenne\`, and because the
...
[Doctordefector/scoop-lyrenne](https://github.com/Doctordefector/scoop-lyrenne), at
`bucket/lyrenne.json`. `packaging/scoop/` in this repo holds no copy: a second one
would go stale the first time Excavator ran.

## Manifest facts that are easy to get wrong

- **Zip root is `Lyrenne/`.** The Gradle task runs `7z a -tzip <zip> "Lyrenne/*"`, so every
  entry carries that prefix. Hence `extract_dir: Lyrenne`. If the zip layout ever changes, the
  manifest breaks.
...
- **Hash case.** Scoop wants lowercase SHA256. Get it without re-downloading 180 MB:
  ```bash
  gh release view vX.Y.Z --repo Doctordefector/Lyrenne --json assets --jq '.assets[].digest'
  ```
...
`scoop update lyrenne` instead. If it ever becomes a real support burden, the fix is to
...
a PR adding `bucket/lyrenne.json`, titled `lyrenne: Add version X.Y.Z`. Reuse
...
downloads next to `Lyrenne.exe`, so all of it lands inside that package folder. `winget upgrade`
...
recovery path either: `AppPaths.migrateFromAppData` only restores from `%APPDATA%\Lyrenne` when
...
under `WinGet\Packages\` or `scoop\apps\`, store data in `%LOCALAPPDATA%\Lyrenne` instead. Once
